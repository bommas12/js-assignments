...
function getPolynom(...args) {
  ...
  return (x) => {
    return args.reduceRight((prev, cur, index) => {
      prev += cur * Math.pow(x, args.length - index - 1);
      return prev;
    }, 0);
  };
}
...
function memoize(func) {
  ...
  return (...args) => {
    const key = args.reduce((prev, cur) => {
      prev += JSON.stringify(cur) + ",";
      return prev;
    }, "");
    let result;
    if (cacheValues.has(key)) {
      result = cacheValues.get(key);
    } else {
      result = func(...args);
      cacheValues.set(key, result);
    }
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    let argsString = args.map((item) => JSON.stringify(item)).join(",");
